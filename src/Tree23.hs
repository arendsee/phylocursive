...
data Tree23 a
  ...
  | Node4 a a a (Tree23 a) (Tree23 a) (Tree23 a) (Tree23 a) -- transient, should never exist in the final tree
...
insert x (Node2 y Nil23 Nil23)
  ...
  | otherwise = Node3 y x empty empty empty
...
insert x (Node2 y lhs rhs)
  ...
  | otherwise = unfour (Node2 y lhs (insert x rhs))
-- node3
insert x (Node3 y z lhs mid rhs)
  | x < y     = unfour (Node3 y z (insert x lhs) mid rhs)
  | x < z     = unfour (Node3 y z lhs (insert x mid) rhs)
  | otherwise = unfour (Node3 y z lhs mid (insert x rhs))
insert _ _ = error "Node4 should not exist"
...
unfour (Node2 n1 (Node4 n2 n3 n4 x1 x2 x3 x4) x5) =
...
-- insert x (Node2 y lhs rhs)
--   | x < y = Node3 y (insert x lhs) rhs
-- insert x (Node3 y z t1 t2 t3) =
